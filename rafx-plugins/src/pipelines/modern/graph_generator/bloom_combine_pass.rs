    ...
    context.graph.set_renderpass_callback(node, move |args| {
        ...
        let mut bloom_combine_material_dyn_set = descriptor_set_allocator
            .create_dyn_descriptor_set(
                &descriptor_set_layouts[bloom_combine_adv_frag::IN_COLOR_DESCRIPTOR_SET_INDEX],
                bloom_combine_adv_frag::DescriptorSet0Args {
                    in_color: &sdr_image,
                    in_blur: &hdr_image,
                    config: &bloom_combine_adv_frag::ConfigStd140 {
        ...
        bloom_combine_material_dyn_set.0.set_buffer(
            bloom_combine_adv_frag::HISTOGRAM_RESULT_DESCRIPTOR_BINDING_INDEX as u32,
            &histogram_result,
        );
