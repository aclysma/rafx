...
use crate::{RenderPhase, RenderView, RenderRegistry, RenderFeatureIndex, RenderPhaseIndex, RenderViewIndex};
use fnv::FnvHashMap;
...
impl FeatureSubmitNodes {
    ...
    pub fn add_submit_node<RenderPhaseT: RenderPhase>(
        ...
        self.submit_nodes[view.view_index() as usize].add_submit_node::<RenderPhaseT>(submit_node_id, sort_key, distance_from_camera);
    }
...
impl MergedFrameSubmitNodes {
    pub fn new(
        ...
        for view in views {
            ...
            for render_phase_index in 0..RenderRegistry::registered_render_phase_count() {
                ...
                log::debug!("Merging submit nodes for view: {} phase: {}", view.view_index(), render_phase_index);
                for fsn in &feature_submit_nodes {
                    ...
                    for submit_node in submit_nodes {
                        log::debug!("submit node feature: {} id: {}", submit_node.feature_index, submit_node.submit_node_id);
                        combined_nodes_for_phase.push(*submit_node);
                    }
                }

                //registry.sort(render_phase_index, &mut combined_nodes_for_phase);
    ...
    pub fn submit_nodes<PhaseT: RenderPhase>(&self, view: &RenderView) -> &[SubmitNode] {
