...
fn determine_memory_layout(binding_struct_type: BindingStructType) -> MemoryLayout {
    match binding_struct_type {
        BindingStructType::Uniform => MemoryLayout::Std140,
        BindingStructType::Buffer => MemoryLayout::Std430,
        BindingStructType::PushConstant => MemoryLayout::Std430,
    }
}
...
fn rust_header(rust_code: &mut Vec<String>) {
    ...
    rust_code.push("use serde::{Deserialize, Serialize};\n\n".to_string());
    rust_code.push("#[allow(unused_imports)]\n".to_string());
...
fn rust_binding_wrappers(
    ...
    for (set_index, bindings) in bindings_by_set {
        ...
        for item in &binding_wrapper_items {
            rust_code.push(format!(
                "    pub {}: {},\n",
                item.binding_name, item.type_name_with_lifetime
            ));
        }
        rust_code.push("}\n\n".to_string());
        ...
        for item in &binding_wrapper_items {
            rust_code.push(format!(
                "        descriptor_set.{}({}, args.{});\n",
                item.setter_fn_name, item.binding_index_string, item.binding_name
            ));
        }
        rust_code.push("    }\n\n".to_string());
        ...
        for item in &binding_wrapper_items {
            rust_code.push(format!(
                ...
                item.binding_name, item.binding_name, item.type_name
            ));
            rust_code.push(format!(
                "        self.0.{}({}, {});\n",
                item.setter_fn_name, item.binding_index_string, item.binding_name
...
struct BindingWrapperItem {
    binding_name: String,
    setter_fn_name: String,
    type_name: String,
    type_name_with_lifetime: String,
    // Likely to be a constant rather than a literal number
    ...
    match e.descriptor_type {
        ...
        DescriptorType::SampledImage | DescriptorType::StorageImage => {
            binding_wrapper_items.push(BindingWrapperItem {
                binding_name,
                setter_fn_name: "set_image".to_string(),
                type_name: "ResourceArc<ImageViewResource>".to_string(),
                type_name_with_lifetime: "ResourceArc<ImageViewResource>".to_string(),
        ...
        DescriptorType::UniformBuffer | DescriptorType::StorageBuffer => {
            ...
            binding_wrapper_items.push(BindingWrapperItem {
                binding_name,
                setter_fn_name: "set_buffer_data".to_string(),
                type_name: format!("&{}", type_name),
                type_name_with_lifetime: format!("&'a {}", type_name),
                binding_index_string,
            });
...
fn get_rust_type_name_alias(
    ...
    name: &str,
    array_sizes: &[usize],
    binding_struct_type: BindingStructType,
) -> Result<String, String> {
