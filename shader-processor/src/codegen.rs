use crate::parse_declarations::{
    BindingType, ParseDeclarationsResult, ParseFieldResult,
    ParsedBindingWithAnnotations,
};
use fnv::FnvHashMap;
use std::sync::Arc;
...
fn create_builtin_type_lookup() -> FnvHashMap<String, TypeAlignmentInfo> {
    ...
    add_type_alignment_info::<bool>(&mut builtin_types, "bool", "bool");
...
pub(crate) fn generate_rust_code(
    ...
    // for (type_name, _user_type) in &user_types {
    //     println!("std140 Type info for {}", type_name);
    //     let alignment = determine_gpu_alignment(&builtin_types, &user_types, type_name, &[], GpuMemoryLayout::Std140)?;
    //     let size = determine_gpu_size(&builtin_types, &user_types, type_name, &[], 0, 0, &type_name, GpuMemoryLayout::Std140)?;
    //     println!("  {}: {} {}", type_name, alignment, size);
    //
    //     println!("std430 Type info for {}", type_name);
    //     let alignment = determine_gpu_alignment(&builtin_types, &user_types, type_name, &[], GpuMemoryLayout::Std430)?;
    //     let size = determine_gpu_size(&builtin_types, &user_types, type_name, &[], 0, 0, &type_name, GpuMemoryLayout::Std430)?;
    //     println!("  {}: {} {}", type_name, alignment, size);
    // }
...
impl GenerateStructResult {
    fn generate_struct_code(&self) -> String {
        ...
        result_string += &format!("#[derive(Copy, Clone, Debug, Default)]\n#[repr(C)]\npub struct {} {{\n", self.name);
...
fn format_member(name: &str, ty: &str, offset: usize, size: usize) -> String {
...
fn generate_struct(
    ...
    for f in &*user_type.fields {
        //
        // Check that the raw size of the field is the same. This may imply padding needed within
        ...
        //
        {
            ...
            if gpu_size_without_alignment != rust_size_without_alignment {
                return Err(format!(
                    ...
                    rust_size_without_alignment,
                    gpu_size_without_alignment
                ));
            }
        }
    ...
    if !type_name.is_empty() {
        ...
        if block.padded_size != 0 {
            ...
            if size != block.size as usize {
                ...
                fn print_block_members(reflect_block_variable: &spirv_reflect::types::ReflectBlockVariable) {
