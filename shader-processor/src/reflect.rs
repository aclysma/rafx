...
use renderer_assets::assets::reflect::{ReflectedEntryPoint, ReflectedDescriptorSetLayoutBinding, ReflectedDescriptorSetLayout, ReflectedPushConstant};

use renderer_resources::vk_description as dsc;
use spirv_cross::spirv::Type;

fn get_descriptor_count_from_type<TargetT>(ast: &spirv_cross::spirv::Ast::<TargetT>, ty: u32) -> Result<u32, String>
    where
        ...
        spirv_cross::spirv::Ast::<TargetT>: spirv_cross::spirv::Compile<TargetT>
{
    ...
    Ok(match ast.get_type(ty).map_err(|_x| "could not get type from reflection data")? {
        Type::Unknown => 0,
        Type::Void => 0,
        ...
        spirv_cross::spirv::Ast::<TargetT>: spirv_cross::spirv::Compile<TargetT>
{
    Ok(match descriptor_type {
        dsc::DescriptorType::UniformBuffer | dsc::DescriptorType::UniformBufferDynamic | dsc::DescriptorType::StorageBuffer | dsc::DescriptorType::StorageBufferDynamic => {
            (ast.get_declared_struct_size(resource.type_id).map_err(|_x| "could not get size from reflection data")? + 15) / 16 * 16
        },
        _ => 0
    })
}
        ...
        spirv_cross::spirv::Ast::<TargetT>: spirv_cross::spirv::Compile<TargetT>
{
    ...
    let set = ast.get_decoration(resource.id, spirv_cross::spirv::Decoration::DescriptorSet).map_err(|_x| "could not get descriptor set index from reflection data")?;
    let binding = ast.get_decoration(resource.id, spirv_cross::spirv::Decoration::Binding).map_err(|_x| "could not get descriptor binding index from reflection data")?;
    ...
    let size = get_descriptor_size_from_resource(ast, resource, descriptor_type).map_err(|_x| "could not get size from reflection data")?;

    let internal_buffer_per_descriptor_size = if parsed_binding.annotations.use_internal_buffer.is_some() {
        Some(size)
    } else {
        None
    };
        ...
        spirv_cross::spirv::Ast::<TargetT>: spirv_cross::spirv::Compile<TargetT>
{
    for resource in resources {
        descriptors.push(get_descriptor_from_resource(ast, declarations, resource, descriptor_type, stage_flags)?);
    }
        ...
        spirv_cross::spirv::Ast::<TargetT>: spirv_cross::spirv::Compile<TargetT>
{
    ...
    for entry_point in ast.get_entry_points().map_err(|_x| "could not get entry point from reflection data")? {
        ...
        let shader_resources = ast.get_shader_resources().map_err(|_x| "could not get resources from reflection data")?;

        get_descriptors_from_resources(&mut bindings, ast, declarations, &shader_resources.uniform_buffers, dsc::DescriptorType::UniformBuffer, stage_flags)?;
        get_descriptors_from_resources(&mut bindings, ast, declarations, &shader_resources.storage_buffers, dsc::DescriptorType::StorageBuffer, stage_flags)?;
        get_descriptors_from_resources(&mut bindings, ast, declarations, &shader_resources.storage_images, dsc::DescriptorType::StorageImage, stage_flags)?;
        get_descriptors_from_resources(&mut bindings, ast, declarations, &shader_resources.sampled_images, dsc::DescriptorType::CombinedImageSampler, stage_flags)?;
        get_descriptors_from_resources(&mut bindings, ast, declarations, &shader_resources.separate_images, dsc::DescriptorType::SampledImage, stage_flags)?;
        get_descriptors_from_resources(&mut bindings, ast, declarations, &shader_resources.separate_samplers, dsc::DescriptorType::Sampler, stage_flags)?;
        // stage inputs
        // stage outputs
        // subpass inputs
        // atomic counters
        ...
        for push_constant in &shader_resources.push_constant_buffers {
            let push_constant_ranges = ast.get_active_buffer_ranges(push_constant.id).map_err(|_x| "could not get active buffer ranges")?;
...
fn map_shader_stage_flags(shader_stage: spirv_cross::spirv::ExecutionModel) -> Result<dsc::ShaderStageFlags, String> {
