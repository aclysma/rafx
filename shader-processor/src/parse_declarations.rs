...
#[derive(Default, Deserialize, Debug)]
#[serde(rename = "export")]
pub(crate) struct UseInternalBufferAnnotation(pub(crate) usize);
...
impl StructAnnotations {
    fn new(annotations: &[Annotation]) -> Result<Self, String> {
        ...
        for annotation in annotations {
            let mut position = 0;
            let annotation_name =
                crate::parse::try_consume_identifier(&annotation.text, &mut position)
...
fn parse_array_sizes(
    ...
    while crate::parse::try_consume_literal(code, position, "[").is_some() {
        ...
        let array_index = crate::parse::try_consume_array_index(code, position).ok_or(format!(
            "Invalid array count while parsing struct field:\n{}",
            crate::parse::characters_to_string(&code)
        ))?;
        array_sizes.push(array_index);
        crate::parse::skip_whitespace(code, position);
        crate::parse::try_consume_literal(code, position, "]").ok_or(format!(
...
fn parse_field(
    ...
    let field_type_name = crate::parse::try_consume_identifier(code, position).ok_or(format!(
    ...
    let field_name = crate::parse::try_consume_identifier(code, position).ok_or(format!(
...
fn try_parse_struct(code: &[char]) -> Result<Option<ParseStructResult>, String> {
    ...
    let type_name = crate::parse::try_consume_identifier(code, &mut position).ok_or(format!(
    ...
    crate::parse::skip_whitespace(code, &mut position);
    crate::parse::try_consume_literal(code, &mut position, ";").ok_or(format!(
...
pub(crate) struct ParseBindingResult {
    pub(crate) layout_parts: Vec<LayoutPart>,
...
fn parse_layout_part(
    ...
    crate::parse::skip_whitespace(code, position);
    let key = crate::parse::try_consume_identifier(code, position).ok_or(format!(
    ...
    if crate::parse::try_consume_literal(code, position, "=").is_some() {
        crate::parse::skip_whitespace(code, position);
        let value = crate::parse::try_consume_identifier(code, position).ok_or(format!(
...
fn try_parse_binding(code: &[char]) -> Result<Option<ParseBindingResult>, String> {
    ...
    let binding_type = crate::parse::try_consume_identifier(code, &mut position).ok_or(format!(
    ...
    crate::parse::skip_whitespace(code, &mut position);
    let instance_name =
        crate::parse::try_consume_identifier(code, &mut position).ok_or(format!(
