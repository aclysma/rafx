...
use rafx::api::{RafxResult, RafxVertexBufferBinding, RafxTextureBarrier, RafxResourceState, RafxCmdCopyBufferToTextureParams, RafxIndexBufferBinding, RafxIndexType};
...
impl FeatureCommandWriter for TextCommandWriter {
    fn on_begin_execute_graph(
        ...
        for image_update in &self.image_updates {
            ...
            log::debug!("upload font atlas data {} bytes", image_update.upload_buffer.get_raw().buffer.buffer_def().size);
            ...
            if rafx_image.texture_def().mip_count > 1 {
                rafx::api::extra::mipmaps::generate_mipmaps(&*write_context.command_buffer, rafx_image)?;
            }
    ...
    fn render_element(
        ...
        self.per_font_descriptor_sets[draw_call.font_descriptor_index as usize].bind(command_buffer)?;
