...
impl RafxDescriptorSetArrayMetal {
    ...
    pub(crate) fn new(
        ...
        {
            let argument_descriptors: Vec<_> =
                argument_descriptors.iter().map(|x| x.into()).collect();
            ...
            if device_context.device_info().debug_names_enabled {
                buffer.set_debug_name(format!("Metal ArgBuffer set {}", layout_index));
            }

            // Static samplers are embedded in shaders generated by spirv_cross
